...
from ..diagnostics import CATEGORY_DATA_PIPELINE, Logger
...
class DataPipeline:
    ...
    def _run(self, url: str, complete: Callable[[PipelineResult | None], None]) -> None:
        ...
        try:
            ...
            Logger.warning(f"data pipeline: '{url}': {exc}", category=CATEGORY_DATA_PIPELINE)
        complete(result)
    ...
    def _resolve(self, url: str) -> PipelineResult | None:
        ...
        if data is not None:
            Logger.info(f"data pipeline: memory '{url}'", category=CATEGORY_DATA_PIPELINE)
            return PipelineResult(data, _JSON, cache_control="no-store")

        out_path = self._out_dir / path
        data = _read_file(out_path)
        if data is not None:
            Logger.info(f"data pipeline: out path '{url}', {out_path}", category=CATEGORY_DATA_PIPELINE)
            return PipelineResult(data, _JSON, cache_control="no-store")
        ...
        if self._in_dir is not None:
            ...
            if data is not None:
                Logger.info(f"data pipeline: in path '{url}', {in_path}", category=CATEGORY_DATA_PIPELINE)
                return PipelineResult(data, _JSON, cache_control="no-store")
...
def _from_network(url: str) -> PipelineResult | None:
    try:
        Logger.info(f"data pipeline: network '{url}'", category=CATEGORY_DATA_PIPELINE)
        ...
        Logger.warning(f"data pipeline: network error '{url}': {exc}", category=CATEGORY_DATA_PIPELINE)
        return None
...
def _read_file(path: Path) -> bytes | None:
    try:
        ...
        Logger.warning(f"data pipeline: file read '{path}': {exc}", category=CATEGORY_DATA_PIPELINE)
    return None
