...
**`"debug"` is opt-in-only, unlike every other group value.** An area is included when its `group` array is a **superset** of `groupFilter` (AND, not OR — `groupFilter === null` imposes no requirement, so every non-debug area shows) **AND** either the area isn't tagged `"debug"`, or `"debug"` is itself part of `groupFilter`. Concretely: a `debug`-tagged area is hidden by default (`groupFilter === null`) and hidden under any *other* explicit filter that doesn't itself include `"debug"` (e.g. `?group=Europe` hides an area tagged `["debug", "Europe"]` too) — it only ever shows when `"debug"` is explicitly requested, either via `?group=debug,...` or the `?debug=1` shorthand. This is why `"debug"` counts as "magic" despite the doc above calling it just a convention: every other group name is visible by default and only needs an *unrelated* filter to hide it, but `"debug"` needs to be affirmatively asked for.
...
**`?logCategoryExclude=`.** `settings.json`'s `excludedCategories` field (see `docs/CLI.md`) is forwarded as `?logCategoryExclude=<comma-joined>` whenever non-empty — independent of whether `?logCategory=` itself is present, since exclusion is geo-builder's own separate signal (unlike `?logCategory=`, it is sent even without an explicit `logCategories`). geo-builder only emits this param; the combination semantics below are geo-browser's own interpretation, same as the existing `group` query param's AND/OR semantics were left to geo-browser to decide.

geo-browser's chosen semantics (`DefaultLogger.excludedCategories` in `src/logging.ts`, wired from `Context.parseLogCategoryExclude()`): exclusion is unconditional and always wins — checked before either `showAllCategories` or the `?logCategory=` allow-list, so an excluded category is suppressed even under `?debug=1`'s show-everything mode, and even if the same category also appears in an explicit `?logCategory=`. There is no special-casing for `general` — excluding it suppresses uncategorized calls too, same as any other category. This mirrors the project's existing "no special-casing" stance on `?logCategory=`'s unrecognized values above.
