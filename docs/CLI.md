...
| `logCategories` | `["general"]` if `debug` is `false`, else `[]` | Restricts stdout logging to these categories only; `[]` means no filtering (every category shown). If left empty/absent, the effective default follows `debug`: `debug: false` (the common case) shows only `general`, matching geo-browser's own default-to-`general` behavior; `debug: true` shows everything. Setting `logCategories` explicitly always wins over this debug-based default, regardless of `debug`'s value. Combined with `logLevel`: a record must pass both filters. Each printed line is tagged `[LEVEL][category]` (e.g. `[WARNING][data_pipeline] ...`). Not a closed set — geo-builder's own categories (`data_pipeline`, `api`, `general`) are just plain constants for reuse; records forwarded from geo-browser (via `WriteTelemetryRecord`) carry whatever category value geo-browser assigned (e.g. `AreaLifecycle`), filterable the same way. **Only when explicitly set (non-empty)**, also forwarded to geo-browser as `?logCategory=<comma-joined>` on `designUrl` (see `docs/MESSAGING.md`) — the debug-gated default is console-only and never sent, since geo-browser's own defaults already match it without any query param. Unrecognized category names (e.g. Python-only ones) are not an error on the geo-browser side — they simply match nothing. |
| `excludedCategories` | `[]` | Deny-list, complementary to `logCategories`. Only has effect when the *effective* `logCategories` is `[]` (the true unfiltered state — `debug: true` with no explicit `logCategories`): in that case stdout shows every category except these. Inert in every other case — an explicit non-empty `logCategories` (or the plain `debug: false` default of `["general"]`) is never narrowed by it, even if the same category name appears in both. Whenever `excludedCategories` is itself non-empty, it is forwarded to geo-browser as `?logCategoryExclude=<comma-joined>` on `designUrl` (see `docs/MESSAGING.md`) — unlike `?logCategory=`, this is sent regardless of whether `logCategories` was explicit, since exclusion is an independent signal. |
