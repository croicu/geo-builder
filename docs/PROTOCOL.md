...
`settings.debug: true` disables all `GeoError` catch blocks so exceptions propagate with full tracebacks. It also writes per-task snapshots to `./build/{task_type}/{counter:03d}/` (see **Debug Output** below), opens a WebView2 remote-debugging port in designer mode, and appends `?debug=1` to `designUrl`. `settings.debug` itself has no effect on the catalog file layout and doesn't stamp or filter areas — that's `settings.group`'s job. Whether the resulting `?debug=1` query param *also* affects which areas geo-browser shows is up to geo-browser (it currently treats it as a back-compat shorthand — see `docs/MESSAGING.md`); geo-builder just emits the param and doesn't implement or depend on that behavior.
